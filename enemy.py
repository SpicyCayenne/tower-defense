...
class Enemy:
    ...
    def __init__(self, img, x, y, display_surface):
        ...
        self.waypoints = itertools.cycle(Enemy.path)
        self.target = next(self.waypoints)
        self.speed = 0.5

    def spawn(self):
        """Spawns new enemy"""
        Enemy.enemies.append(Enemy(self.img_file, self.x_spawn, self.y_spawn, self.display_surface))

    def move(self):
        """Moves the enemy down the path"""
        distance_to_target = math.sqrt((self.x_coord - self.target[0])**2 +
                                       (self.y_coord - self.target[1])**2)
        if distance_to_target <= 0.5 and next(self.waypoints) != Enemy.path[0]:
            self.target = next(self.waypoints)
        ...
        if (self.x_coord, self.y_coord) == Enemy.path[-1]:
